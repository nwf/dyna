---------------------------------------------------------------------------
-- | Parser self-test cases
--
-- TODO:
--   Writing these is still too hard, Template Haskell and the REPL
--     notwithstanding.
--
--   Test.Framework.TH appears not to understand block comments at the
--   moment, and parses right through them.

-- Header material                                                      {{{

{-# LANGUAGE TemplateHaskell #-}
{-# LANGUAGE Rank2Types #-}
{-# LANGUAGE OverloadedStrings #-}

module Dyna.ParserHS.Selftest where

import           Control.Applicative
import           Data.ByteString (ByteString)
import qualified Data.ByteString                     as B
import qualified Data.ByteString.UTF8                as BU
import           Data.Functor.Foldable(Fix(..))
import qualified Data.Map                            as M
import           Data.String
-- import           Dyna.ParserHS.OneshotDriver
import           Dyna.ParserHS.Parser
import           Dyna.ParserHS.Printer
import           Dyna.ParserHS.Syntax
import           Dyna.ParserHS.Types
import           Dyna.Term.TTerm (TBase(..))
import           Dyna.XXX.Trifecta (unSpan)
import           Dyna.XXX.TrifectaTest
import           Test.Framework                      as TF
import           Test.Framework.Providers.HUnit
import           Test.Framework.Providers.QuickCheck2
import           Test.Framework.TH
import           Test.HUnit                          as H
import           Test.QuickCheck                     as Q
import qualified Text.PrettyPrint.Free               as PP
import           Text.Trifecta
import           Text.Trifecta.Delta


------------------------------------------------------------------------}}}
-- Terms and basic handling                                             {{{

_tNumeric :: Either Integer Double -> Term
_tNumeric = Term . TPrim . TNumeric

_tPrefix :: ByteString -> Spanned Term -> Term
_tPrefix f a = Term (TPrefix f a)

_tPostfix :: ByteString -> Spanned Term -> Term
_tPostfix f a = Term (TPostfix f a)

_tInfix :: ByteString -> Spanned Term -> Spanned Term -> Term
_tInfix f a b = Term (TInfix f a b)

_tFunctor :: ByteString -> [Spanned Term] -> Term
_tFunctor f as = Term (TFunctor f as)

_tVar :: ByteString -> Term
_tVar = Term . TVar

testDLC :: DLCfg
testDLC = mkDLC dynaOperSpec

term :: Parser (Spanned Term)
term   = testTerm testDLC <* eof

upterm :: ByteString -> Spanned Term
upterm = unsafeParse term

recunspan :: Spanned Term -> Fix TermF
recunspan ((Term t) :~ _) = Fix (fmap recunspan t)

case_basicAtom :: Assertion
case_basicAtom = e @=? (upterm "foo")
 where e = _tFunctor "foo" []
            :~ Span (Columns 0 0) (Columns 3 3) "foo"

case_booleans_not_atoms :: Assertion
case_booleans_not_atoms = map snd t @=? map (unSpan.upterm.fst) t
 where t = [ ("true"   , Term $ TPrim $ TBool True)
           , ("false"  , Term $ TPrim $ TBool False)
           , ("'true'" , Term $ TFunctor "true"  [])
           , ("'false'", Term $ TFunctor "false" [])
           ]

case_basicAtomTWS :: Assertion
case_basicAtomTWS = e @=? (upterm "foo ")
 where e =  _tFunctor "foo" [] :~ Span (Columns 0 0) (Columns 4 4) "foo "

case_basicFunctor :: Assertion
case_basicFunctor = e @=? (upterm sfb)
 where
  e = _tFunctor "foo"
         [ _tFunctor "bar" [] :~ Span (Columns 4 4) (Columns 7 7) sfb
         ]
       :~ Span (Columns 0 0) (Columns 8 8) sfb

  sfb :: (IsString s) => s
  sfb = "foo(bar)"

case_dollarFunctor :: Assertion
case_dollarFunctor = e @=? (upterm sfb)
 where
  e = _tFunctor "$foo"
         [ _tFunctor "bar" [] :~ Span (Columns 5 5) (Columns 8 8) sfb
         ]
       :~ Span (Columns 0 0) (Columns 9 9) sfb

  sfb :: (IsString s) => s
  sfb = "$foo(bar)"

case_nestedFunctorsWithArgs :: Assertion
case_nestedFunctorsWithArgs = e @=? (upterm st)
 where
  e = _tFunctor "foo"
         [ _tFunctor "bar" [] :~ Span (Columns 4 4) (Columns 7 7) st
         , _tVar "X" :~ Span (Columns 8 8) (Columns 9 9) st
         , _tFunctor "bif" [] :~ Span (Columns 10 10) (Columns 15 15) st
         , _tFunctor "baz"
              [ _tFunctor "quux" [] :~ Span (Columns 20 20) (Columns 24 24) st
              , _tVar "Y" :~ Span (Columns 25 25) (Columns 26 26) st
              ]
            :~ Span (Columns 16 16) (Columns 27 27) st
        ]
       :~ Span (Columns 0 0) (Columns 28 28) st

  st :: (IsString s) => s
  st = "foo(bar,X,bif(),baz(quux,Y))"

case_basicFunctorComment :: Assertion
case_basicFunctorComment = e @=? (upterm sfb)
 where
  e = _tFunctor "foo" [] :~ Span (Columns 0 0) (Columns 8 8) sfb
 
  sfb :: (IsString s) => s
  sfb = "foo %xxx"

case_basicFunctorNLComment :: Assertion
case_basicFunctorNLComment = e @=? (upterm sfb)
 where
  e = Term
       (TFunctor "foo"
         [_tNumeric (Left 1) :~ Span (Lines 1 0 9 0) (Lines 1 1 10 1) "1,2\n"
         ,_tNumeric (Left 2) :~ Span (Lines 1 2 11 2) (Lines 2 0 13 0) "1,2\n"
         ])
       :~ Span (Columns 0 0) (Lines 2 1 14 1) "foo(%xxx\n"

  sfb :: (IsString s) => s
  sfb = "foo(%xxx\n1,2\n)"

case_basicFunctorTWS :: Assertion
case_basicFunctorTWS = e @=? (upterm sfb)
 where
  e = _tFunctor "foo"
         [ _tFunctor "bar" [] :~ Span (Columns 5 5) (Columns 9 9) sfb
         ]
       :~ Span (Columns 0 0) (Columns 10 10) sfb

  sfb :: (IsString s) => s
  sfb = "foo (bar )"

case_basicFunctorNL :: Assertion
case_basicFunctorNL = e @=? (upterm sfb)
 where
  e = _tFunctor "foo"
         [ _tFunctor "bar" [] :~ Span (Lines 1 1 5 1) (Lines 1 5 9 5) "(bar )"
         ]
       :~ Span (Columns 0 0) (Lines 1 6 10 6) "foo\n"

  sfb :: (IsString s) => s
  sfb = "foo\n(bar )"

-- XXX This example is taken from our 2011 paper, but I am not sure that
-- infix colon is really by default just a term constructor?  It's possible,
-- to be sure!  If it's not a built-in, then we should move this to some
-- other case where we're also testing that we can adjust the operator
-- table.
--
--case_colonFunctor :: Assertion
--case_colonFunctor = e @=? (upterm pvv)
-- where
--  e = TFunctor "possible"
--        [TFunctor ":"
--           [TVar "Var" :~ Span (Columns 9 9) (Columns 12 12) pvv
--           ,TVar "Val" :~ Span (Columns 13 13) (Columns 16 16) pvv
--           ]
--          :~ Span (Columns 9 9) (Columns 16 16) pvv
--        ]
--       :~ Span (Columns 0 0) (Columns 17 17) pvv
--  pvv = "possible(Var:Val)"

case_quote_vs_ticked_quote :: Assertion
case_quote_vs_ticked_quote = map snd t @=? map (recunspan . upterm . fst) t
 where
  t = [ ("(&a)"    , Fix $ TPrefix  "&" (Fix $ TFunctor "a" []))
      , ("(& a)"   , Fix $ TPrefix  "&" (Fix $ TFunctor "a" []))
      , ("('&'(a))", Fix $ TFunctor "&" [Fix $ TFunctor "a" []])
      ]

case_longest_match_tricky_cases :: Assertion
case_longest_match_tricky_cases = map snd t @=? map (recunspan . upterm . fst) t
 where
  t = [ ("(!\"x\")"        , Fix $ TPrefix "!"   (Fix $ TPrim (TString "x")))
      , ("(&(a))"          , Fix $ TPrefix "&"   (Fix $ TFunctor "a" []))
      , ("(a for(b))"      , Fix $ TInfix  "for" (Fix $ TFunctor "a" [])
                                                 (Fix $ TFunctor "b" []))
      ]

case_nullaryStar :: Assertion
case_nullaryStar = e @=? (upterm gs)
 where
  e  = _tFunctor "gensym"
         [_tFunctor "*" [] :~ Span (Columns 7 7) (Columns 8 8) gs]
        :~ Span (Columns 0 0) (Columns 9 9) gs
  gs = "gensym(*)"

------------------------------------------------------------------------}}}
-- Expressions                                                          {{{

case_basicFunctorExpr :: Assertion
case_basicFunctorExpr = e @=? (upterm s)
 where
  e = _tFunctor "f"
        [_tInfix "+"
           (_tFunctor "a" [] :~ Span (Columns 2 2) (Columns 3 3) s)
           (_tFunctor "b" [] :~ Span (Columns 4 4) (Columns 5 5) s)
           :~ Span (Columns 2 2) (Columns 5 5) s
        ]
        :~ Span (Columns 0 0) (Columns 6 6) s
  s = "f(a+b)"

-- Regression for github nwf/dyna#45
case_operQuote :: Assertion
case_operQuote = e @=? (upterm s)
 where
  e = _tInfix "!="
        (_tFunctor "a" [] :~ Span (Columns 1 1) (Columns 3 3) s)
        (_tPrefix "&" 
           (_tFunctor "b" [] :~ Span (Columns 7 7) (Columns 8 8) s)
              :~ Span (Columns 6 6) (Columns 8 8) s)
       :~ Span (Columns 1 1) (Columns 8 8) s
  s = "(a != &b)"

case_failIncompleteExpr :: Assertion
case_failIncompleteExpr = checkParseFail term "foo +"
  (\s -> take 18 s @=? "(interactive):1:5:")


------------------------------------------------------------------------}}}
-- List handling                                                        {{{

case_list :: Assertion
case_list = e @=? (upterm s)
 where
  e = _tFunctor dynaConsOper
       [_tNumeric (Left 1) :~ Span (Columns 1 1) (Columns 2 2) s
       ,_tFunctor dynaConsOper
         [_tInfix "+"
           (_tNumeric (Left 2) :~ Span (Columns 3 3) (Columns 4 4) s)
           (_tNumeric (Left 3) :~ Span (Columns 5 5) (Columns 6 6) s)
           :~ Span (Columns 3 3) (Columns 6 6) s
         ,_tFunctor dynaNilOper [] :~ Span (Columns 7 7) (Columns 7 7) s]
         :~ Span (Columns 3 3) (Columns 7 7) s]
       :~ Span (Columns 0 0) (Columns 7 7) s
  s = "[1,2+3]"

case_list_bar :: Assertion
case_list_bar = e @=? (upterm s)
 where
  e = _tFunctor dynaConsOper
       [_tNumeric (Left 1) :~ Span (Columns 1 1) (Columns 2 2) s
       ,_tFunctor dynaConsOper
         [_tInfix "+"
           (_tNumeric (Left 2) :~ Span (Columns 3 3) (Columns 4 4) s)
           (_tNumeric (Left 3) :~ Span (Columns 5 5) (Columns 6 6) s)
           :~ Span (Columns 3 3) (Columns 6 6) s
         ,_tVar "X" :~ Span (Columns 7 7) (Columns 8 8) s]
         :~ Span (Columns 3 3) (Columns 8 8) s]
       :~ Span (Columns 0 0) (Columns 8 8) s
  s = "[1,2+3|X]"

------------------------------------------------------------------------}}}
-- Annotations                                                          {{{

--case_tyAnnot :: Assertion
--case_tyAnnot = e @=? (strictterm fintx)
-- where
--  e = TFunctor "f" [TAnnot (AnnType $ TFunctor "int" []
--                                     :~ Span (Columns 3 3) (Columns 7 7) fintx)
--                           (TVar "X" :~ Span (Columns 7 7) (Columns 8 8) fintx)
--                     :~ Span (Columns 2 2) (Columns 8 8) fintx
--                   ]
--                  :~ Span (Columns 0 0) (Columns 9 9) fintx
--  fintx = "f(:int X)"

------------------------------------------------------------------------}}}
-- Aggregators                                                          {{{

-- test_prim_aggregators :: [TF.Test]
-- test_prim_aggregators = hUnitTestToTests $ TestList $
--   map (\(s,a) -> s ~: (Left a) ~=? parseAgg (BU.fromString s)) $ M.toList primAggrSpec
--  where
--   parseAgg = unsafeParse (mkAggrParser primAggrSpec)

-- okAggrs :: [B.ByteString]
-- okAggrs = [daPlusEq, "*=", ".=", "min=", "max=", "?=", daProlog, "max+=" ]
--
--test_aggregators :: [TF.Test]
--test_aggregators = hUnitTestToTests $ TestList
--  [ TestLabel "generic valid" $ TestList $
--      map (\x -> (BU.toString x) ~: x ~=? unsafeParse testGenericAggr x)
--          okAggrs
--  , TestLabel "generic invalid" $ TestList $
--      map (\x -> TestLabel (BU.toString x) $ TestCase
--                                           $ checkParseFail
--                                               testGenericAggr
--                                               x
--                                               (\_ -> return ()))
--        [".", ". ", "+=3", "+3=", "+=a", "+a=" ]
--  , TestLabel "custom accept" $
--      let r :~ _ = unsafeParse (testRule cdlc) r1
--      in r ~=? Rule (TFunctor "a" [] :~ Span (Columns 0 0) (Columns 2 2) r1)
--                    daPlusEq
--                    (TFunctor "b" [] :~ Span (Columns 5 5) (Columns 6 6) r1)
--  , TestLabel "custom reject" $ TestCase
--                              $ checkParseFail (testRule cdlc)
--                                               "a *= b."
--                                               (\_ -> return ())
--  ]
-- where
--  r1 = "a += b."
--
--  cdlc = DLC { dlc_opertab = dlc_opertab defDLC
--             , dlc_aggrs   = fmap BU.fromString (string daPlusEq)
--             }

------------------------------------------------------------------------}}}
-- Rules                                                                {{{

progrule :: ByteString -> Spanned Rule
progrule = unsafeParse (whiteSpace *> (testRule testDLC <* eof))

progrules :: ByteString -> [Spanned Rule]
progrules = unsafeParse (whiteSpace *> many (testRule testDLC) <* eof)

--oneshotRules :: ByteString -> [(RuleIx, Spanned Rule)]
--oneshotRules = xlate . unsafeParse (oneshotDynaParser Nothing)
-- where
--  xlate (PDP rs _ _ _) = map (\(i,_,sr) -> (i,sr)) rs
--
case_ruleFact :: Assertion
case_ruleFact = e @=? (progrule sr)
 where
  e  = Rule
       (_tFunctor "goal" [] :~ Span (Columns 0 0) (Columns 4 4) sr)
       daProlog
       (Term (TPrim (TBool True)) :~ Span (Columns 4 4) (Columns 5 5) sr)
      :~ Span (Columns 0 0) (Columns 5 5) sr
  sr = "goal."

case_ruleSimple :: Assertion
case_ruleSimple = e @=? (progrule sr)
 where
  e  = Rule
       (_tFunctor "goal" [] :~ Span (Columns 0 0) (Columns 5 5) sr)
       daPlusEq
       (_tNumeric (Left 1) :~ Span (Columns 8 8) (Columns 9 9) sr)
      :~ Span (Columns 0 0) (Columns 10 10) sr
  sr = "goal += 1."

case_rule_funny_spacing :: Assertion
case_rule_funny_spacing = e @=? (progrule sr)
 where
  e  = Rule
       (_tFunctor "goal" [] :~ Span (Columns 0 0) (Columns 5 5) sr)
       daPlusEq
       (_tFunctor "a" [] :~ Span (Columns 7 7) (Columns 8 8) sr)
      :~ Span (Columns 0 0) (Columns 9 9) sr
  sr = "goal +=a."

case_rule_no_spaces :: Assertion
case_rule_no_spaces = e @=? (progrule sr)
 where
  e  = Rule
       (_tFunctor "goal" [] :~ Span (Columns 0 0) (Columns 4 4) sr)
       daPlusEq
       (_tFunctor "a" [] :~ Span (Columns 6 6) (Columns 7 7) sr)
      :~ Span (Columns 0 0) (Columns 8 8) sr
  sr = "goal+=a."

case_ruleSimple0 :: Assertion
case_ruleSimple0 = e @=? (progrule sr)
 where
  e  = Rule
       (_tFunctor "goal" [] :~ Span (Columns 0 0) (Columns 5 5) sr)
       daPlusEq
       (_tNumeric (Left 0) :~ Span (Columns 8 8) (Columns 9 9) sr)
      :~ ts
  ts = Span (Columns 0 0) (Columns 10 10) sr
  sr = "goal += 0."

case_ruleExpr :: Assertion
case_ruleExpr = e @=? (progrule sr)
 where
  e  = Rule
       (_tFunctor "goal" [] :~ Span (Columns 0 0) (Columns 5 5) sr)
       daPlusEq
       (_tInfix "+"
          (_tFunctor "foo" [] :~ Span (Columns 8 8) (Columns 12 12) sr)
          (_tFunctor "bar" [] :~ Span (Columns 14 14) (Columns 18 18) sr)
          :~ Span (Columns 8 8) (Columns 18 18) sr)
      :~ ts
  ts = Span (Columns 0 0) (Columns 19 19) sr
  sr = "goal += foo + bar ."

-- XXX Move this to some other place where we've augmented the operator
-- table to understand that ".bar" is a postfix operator.
--
--case_ruleDotExpr :: Assertion
--case_ruleDotExpr = e @=? (progrule sr)
-- where
--  e  = Rule
--       (TFunctor "goal" [] :~ Span (Columns 0 0) (Columns 5 5) sr)
--       daPlusEq
--       (TFunctor "."
--            [TFunctor "foo" [] :~ Span (Columns 8 8) (Columns 11 11) sr
--            ,TFunctor "bar" [] :~ Span (Columns 12 12) (Columns 15 15) sr
--            ]
--           :~ Span (Columns 8 8) (Columns 15 15) sr)
--      :~ ts
--  ts = Span (Columns 0 0) (Columns 16 16) sr
--  sr = "goal += foo.bar."


case_ruleComma :: Assertion
case_ruleComma = e @=? (progrule sr)
 where
  e =  Rule
       (_tFunctor "foo" [] :~ Span (Columns 0 0) (Columns 4 4) sr)
       daPlusEq
       (_tInfix ","
         (_tFunctor "bar"
            [_tVar "X" :~ Span (Columns 11 11) (Columns 12 12) sr]
            :~ Span (Columns 7 7) (Columns 13 13) sr)
         (_tInfix ","
            (_tFunctor "baz"
               [_tVar "X" :~ Span (Columns 19 19) (Columns 20 20) sr]
               :~ Span (Columns 15 15) (Columns 21 21) sr)
            (_tVar "X" :~ Span (Columns 23 23) (Columns 24 24) sr)
            :~ Span (Columns 15 15) (Columns 24 24) sr)
         :~ Span (Columns 7 7) (Columns 24 24) sr)
       :~ ts
  ts = Span (Columns 0 0) (Columns 25 25) sr
  sr = "foo += bar(X), baz(X), X."

case_ruleKeywordsComma :: Assertion
case_ruleKeywordsComma = e @=? (progrule sr)
 where
  e = Rule
      (_tFunctor "foo" [] :~ Span (Columns 0 0) (Columns 4 4) sr)
      "="
      (_tInfix "whenever" 
         (_tPrefix "new"
            (_tVar "X" :~ Span (Columns 10 10) (Columns 12 12) sr)
            :~ Span (Columns 6 6) (Columns 12 12) sr)
         (_tInfix ","
            (_tInfix "is"
               (_tVar "X" :~ Span (Columns 21 21) (Columns 23 23) sr)
               (_tFunctor "baz"
                  [_tVar "Y" :~ Span (Columns 30 30) (Columns 31 31) sr]
                  :~ Span (Columns 26 26) (Columns 32 32) sr)
               :~ Span (Columns 21 21) (Columns 32 32) sr)
            (_tInfix "is"
               (_tVar "Y" :~ Span (Columns 34 34) (Columns 36 36) sr)
               (_tNumeric (Left 3) :~ Span (Columns 39 39) (Columns 41 41) sr)
               :~ Span (Columns 34 34) (Columns 41 41) sr)
            :~ Span (Columns 21 21) (Columns 41 41) sr) -- End "whenever"
         :~ Span (Columns 6 6) (Columns 41 41) sr) -- End expression
      :~ ts
  ts = Span (Columns 0 0) (Columns 42 42) sr
  sr = "foo = new X whenever X is baz(Y), Y is 3 ."

case_rules :: Assertion
case_rules = e @=? (progrules sr)
 where
  e = [ Rule
        (_tFunctor "goal" [] :~ Span (Columns 0 0) (Columns 5 5) sr)
        daPlusEq
        (_tNumeric (Left 1) :~ Span (Columns 8 8) (Columns 10 10) sr)
       :~ s1
      , Rule
        (_tFunctor "laog" [] :~ Span (Columns 12 12) (Columns 17 17) sr)
        "min="
        (_tNumeric (Left 2) :~ Span (Columns 22 22) (Columns 24 24) sr)
       :~ s2
      ]
  -- Here and elsewhere, it is important that the rules not abut!  The
  -- whitespace separating them is not to be counted in either one.
  s1 = Span (Columns 0 0) (Columns 11 11) sr
  s2 = Span (Columns 12 12) (Columns 25 25) sr
  sr = "goal += 1 . laog min= 2 ."

--case_rules_with_ruleix_pragmas :: Assertion
--case_rules_with_ruleix_pragmas = e @=? (oneshotRules sr)
-- where
--  e = [ ( 5
--        , Rule
--          (TFunctor "goal" [] :~ Span (Columns 13 13) (Columns 18 18) sr)
--          daPlusEq
--          (_tNumeric (Left 1) :~ Span (Columns 21 21) (Columns 22 22) sr)
--         :~ s1
--        )
--      , ( 6
--        , Rule
--          (TFunctor "laog" [] :~ Span (Columns 24 24) (Columns 29 29) sr)
--          "min="
--          (_tNumeric (Left 2) :~ Span (Columns 34 34) (Columns 35 35) sr)
--        :~ s2
--        )
--      ]
--
--  s1 = Span (Columns 13 13) (Columns 23 23) sr
--  s2 = Span (Columns 24 24) (Columns 36 36) sr
--  sr = ":- ruleix 5. goal += 1. laog min= 2."
--
--case_just_ruleix_pragma :: Assertion
--case_just_ruleix_pragma = [] @=? (oneshotRules ":-ruleix 5.")

case_rulesWhitespace :: Assertion
case_rulesWhitespace = e @=? (progrules sr)
 where
  e  = [ Rule
         (_tFunctor "goal" [] :~ Span (Columns 2 2) (Lines 1 1 16 1) l0)
         daPlusEq
         (_tNumeric (Left 1) :~ Span (Lines 1 4 19 4) (Lines 1 6 21 6) l1)
        :~ s1
       , Rule
         (_tFunctor "goal" [] :~ Span (Lines 3 1 31 1) (Lines 3 6 36 6) l3)
         daPlusEq
         (_tNumeric (Left 2) :~ Span (Lines 3 9 39 9) (Lines 3 11 41 11) l3)
        :~ s2
       ]
  l0 = "  goal%comment\n"
  l1 = " += 1 .\n"
  l2 = "%test \n"
  l3 = " goal += 2 ."
  s1 = Span (Columns 2 2) (Lines 1 7 22 7) l0
  s2 = Span (Lines 3 1 31 1) (Lines 3 12 42 12) l3
  sr = B.concat [l0,l1,l2,l3]

-- XXX This one needs ".bar" as a postfix operator.
--
--case_rulesDotExpr :: Assertion
--case_rulesDotExpr = e @=? (progrules sr)
-- where
--  e  = [ Rule
--         (TFunctor "goal" [] :~ Span (Columns 0 0) (Columns 5 5) sr)
--         daPlusEq
--         (TFunctor "."
--              [TFunctor "foo" [] :~ Span (Columns 8 8) (Columns 11 11) sr
--              ,TFunctor "bar" [] :~ Span (Columns 12 12) (Columns 15 15) sr
--              ]
--             :~ Span (Columns 8 8) (Columns 15 15) sr)
--        :~ s1
--       , Rule
--         (TFunctor "goal" [] :~ Span (Columns 17 17) (Columns 22 22) sr)
--         daPlusEq
--         (_tNumeric (Left 1) :~ Span (Columns 25 25) (Columns 27 27) sr)
--        :~ s2
--       ]
--  s1 = Span (Columns 0 0) (Columns 16 16) sr
--  s2 = Span (Columns 17 17) (Columns 28 28) sr
--  sr = "goal += foo.bar. goal += 1 ."

case_rule_with_unknown_operator :: Assertion
case_rule_with_unknown_operator =
  checkParseFail (testRule $ mkDLC M.empty)
                 "goal += 1 ### 2."
                 (\s -> take 19 s @=? "(interactive):1:11:")

------------------------------------------------------------------------}}}
-- Pragmas                                                              {{{

arbPragma :: Gen Pragma
arbPragma = oneof
  [ PBackchain <$> ((,) <$> arbAtom <*> (getPositive <$> arbitrary))
  , PDispos <$> arbPosn <*> arbSD <*> arbAtom <*> listOf arbAD
  , PDisposDefl <$> arbitraryBoundedEnum -- , "prologish"]
  , PIAggr <$> arbAtom <*> (getPositive <$> arbitrary) <*> (elements primAggrs)
  , PRuleIx <$> (getPositive <$> arbitrary)
  ]
 where
  arbSD = arbitraryBoundedEnum
  arbAD = arbitraryBoundedEnum

  arbAtom = elements [ "f", "+" ]

  arbPosn = arbitraryBoundedEnum

  -- arbFix = elements [ PFPre
  --                   , PFPost
  --                   , PFIn TP.AssocNone
  --                   , PFIn TP.AssocLeft
  --                   , PFIn TP.AssocRight
  --                   ]

  primAggrs = [ daPlusEq, "min=", daProlog, "="]
  -- primAggrs = map Left $ M.elems primAggrSpec

prop_pragma_roundtrip :: Property
prop_pragma_roundtrip =
  forAll arbPragma (\p -> p == unsafeParse (testPragma testDLC)
                      (BU.fromString
                      (flip PP.displayS "" $ PP.renderCompact
                                           $ renderPragma p)))

------------------------------------------------------------------------}}}
-- Harness toplevel                                                     {{{

selftest :: TF.Test
selftest = $(testGroupGenerator)

------------------------------------------------------------------------}}}
